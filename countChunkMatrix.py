...
if len(sys.argv) != 4:
    print 'USAGE: countChunkMatrix.py inputFile outputFile chunkCountingThreshhold'
    print len(sys.argv)
    sys.exit(2)

inputFile = sys.argv[1]
outputFile = sys.argv[2]
chunkCountingThreshhold = float(sys.argv[3]) / 100.0
        ...
        if lineIndex == 0:
            ...
            try:
                ...
                for b in range(fileIndex + 1, len(columns)):
                    fileData[unchunkedName] ['percentTotals'].append(0)

            fileData[unchunkedName] ['numberOfFiles'] += 1

            for a in range(fileIndex + 1, len(columns)):
                if float(columns[a]) >= chunkCountingThreshhold:
                    fileData[unchunkedName] ['percentTotals'][a - fileIndex - 1] += 1
